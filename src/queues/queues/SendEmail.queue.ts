...
import { QueueLogger } from '../../loggers/bullMQ.logger.ts';
...
class SendEmailQueue {
  ...
  private readonly _connection: Redis = connection;
  ...
  public async sendEmailVerify(job: VerifyEmailJobType) {
    ...
    try {
      ...
      throw new Error('Add job sendEmailVerify fail with error: ', error);
    }
  }
}

export const SendEmailQueueInstance = SendEmailQueue.getInstance();
